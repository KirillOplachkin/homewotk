...
mers = Mersedes(model='cls', power=256, weight=2, colour='red')
#print(mers)
...
sport = SportMers(model='cls', power=256, weight=2, colour='red')
print(sport)
print(sport.Colour_PLus('blue'))

class Coast(Mersedes):
    def __init__(self, model, power, weight, colour):
        super().__init__(model, power, weight, colour)
    def coast(self):
        return f'coast is 20000 $'
    def __str__(self):
...
plus_power = Plus_Power(model='cls', power=256, weight=2, colour='red')
print(plus_power)
...
kin_gard = Kinder_garden(number=27, name='Имени горького', age_of_students='2 - 5', number_of_students='270')
print(kin_gard)
...
school = School(number=26, name='Имени Лермонтова', age_of_students='6 - 17', number_of_students='2400', document='Grade 11 attestat', language='Французского')
...
college_1 = College(number=34, name='Имени Ломоносова', age_of_students='16 - 19', number_of_students='2700', document='Среднее профессиональное ', faculty=[
    'ГидроСтроительный',
    'Авиационный',
    'Инженерный'
])
